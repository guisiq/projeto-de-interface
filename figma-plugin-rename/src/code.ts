...
// ─── Screen signatures: unique text markers for each screen ───

interface ScreenSignature {
  name: string;        // Nome correto (ex: "02 — Login")
  requiredTexts: string[];  // Textos que DEVEM estar presentes
  forbiddenTexts?: string[]; // Textos que NÃO podem estar (para desambiguar)
  priority?: number;   // Maior = mais específico, testado primeiro
}

const SCREEN_SIGNATURES: ScreenSignature[] = [
...
function getAllTextContent(node: SceneNode): string {
  ...
  if ("children" in node) {
    return (node as ChildrenMixin & SceneNode).children
      .map((c: SceneNode) => getAllTextContent(c))
      .filter(Boolean)
      .join(" ");
  }
  return "";
...
interface RenameResult {
  ...
  renamed: number;
  unidentified: number;
  details: { oldName: string; newName: string; nodeId: string }[];
  unidentifiedFrames: { name: string; nodeId: string; sampleText: string }[];
}

function autoRenameFrames(): RenameResult {
  ...
  // Phase 1: Score each frame against each signature
  interface Match { frameIdx: number; sigIdx: number; score: number; }
  ...
  const assignedSigs = new Set<number>();

  for (const match of allMatches) {
    if (assignedFrames.has(match.frameIdx)) continue;
    if (assignedSigs.has(match.sigIdx)) continue;
